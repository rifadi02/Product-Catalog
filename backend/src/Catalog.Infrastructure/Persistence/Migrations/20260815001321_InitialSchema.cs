...
namespace Catalog.Infrastructure.Persistence.Migrations
{
    /// <inheritdoc />
    /// <remarks>
    /// <para><b>About the `xmin` line in `products` below — do not "fix" it.</b> The scaffolder
    /// lists the concurrency token as a column because it is a mapped property, but Npgsql's SQL
    /// generator recognises `xmin`/`xid` as a PostgreSQL system column and emits no DDL for it.
    /// Verified with `dotnet ef migrations script`: the generated `CREATE TABLE catalog.products`
    /// has nine columns and no `xmin`. Removing the line by hand would desynchronise the model
    /// snapshot and make the very next migration try to add the column for real.</para>
    /// </remarks>
